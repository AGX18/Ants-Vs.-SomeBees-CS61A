...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if place.ant.can_contain(self):  # the ant in the place is a container ant
...
class ThrowerAnt(Ant):
    ...
    lower_bound = 0  # the place where we're at
    ...
    def nearest_bee(self):
        ...
        beginning = self.place
        lower_and_upper_checker = 0

        while beginning is not None:
            if beginning.is_hive:
                return None

            if self.lower_bound <= lower_and_upper_checker <= self.upper_bound:
                if beginning.bees:
                    return random_bee(beginning.bees)

            beginning = beginning.entrance
            lower_and_upper_checker += 1

        # return None # this line is actually redundant
        # END Problem 3 and 4
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def double(self):
        behind_my_place = self.place.exit
        while behind_my_place is not None:  # doubling the ants' damage
            ant_in_place = behind_my_place.ant
            if ant_in_place is not None:
                if ant_in_place.is_container and ant_in_place.ant_contained is not None:
                    Ant.double(ant_in_place.ant_contained)
                Ant.double(ant_in_place)
            behind_my_place = behind_my_place.exit
